...
- **`EntityComponentEngineContext`** (`entity components/engine_context.js`), on a single permanent entity named `"EngineContext"` built by `main.js`'s `initEngineContext()` before any other entity, holds Three.js's own "bare minimum" objects (`scene`/`sceneHUD`/`renderer`/`camera`/`cameraPivot` so far) so components can fetch them via `EntityComponent.methodGetScene()`/`methodGetSceneHUD()`/`methodGetRenderer()`/`methodGetCamera()`/`methodGetCameraPivot()` instead of receiving them as hard-wired constructor params — see `BARE_MINIMUM_THREEJS_EXCEPTION_OR_NOT.md` for the full design (`cameraHUD` isn't converted yet). Backed by `EntityManager.methodGetEntityByName(paramName)`, a new by-name lookup primitive (delegated through `Entity`/`EntityComponent` the same way `methodGetEntitiesWithComponent` is). **Safe to cache**: a component that reads one of these every frame (e.g. `EntityComponentCameraControllerFirstPerson`) should resolve it once in its own `methodInitialize()` and store the reference in a private field, rather than re-running the lookup chain on every `methodUpdate()` call — every `EngineContext` value is constructed once in `initBareMinimum()` and only ever mutated in place afterward, never replaced by a different object, and the `"EngineContext"` entity itself is never removed, so a cached reference can't go stale. **Reused classes that back both `scene` and `sceneHUD` instances** (`EntityComponentTestCube`/`EntityComponentDirectionalLight`) resolve which one via an overridable `methodGetTargetScene()` hook (defaulting to `methodGetScene()`) rather than a constructor param — `EntityComponentTestCubeHUD` and the new `EntityComponentDirectionalLightHUD` each override it to return `methodGetSceneHUD()` instead.
